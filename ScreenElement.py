...
class healthBar:
    def __init__(self, x, y, width, height):
        self.x = x
        self.y = y
        self.heart_width = height  # Make hearts square based on height
        ...
        self.animation_speed = 200  # Increased for faster animation
        self.animation_timer = 0
        self.calculate_heart_states()
        
    def calculate_heart_states(self):
        """Calculate target heart states based on current life"""
        health_per_heart = 20  # 100 health / 5 hearts = 20 health per heart
        ...
        for i in range(5):
            ...
            if self.life >= heart_health_end:
                self.target_states[i] = 0  # Full heart
            elif self.life <= heart_health_start:
                self.target_states[i] = 2  # Empty heart
            else:
                # Half heart - when health is between start and end of this heart
                remaining_in_heart = self.life - heart_health_start
                if remaining_in_heart >= health_per_heart // 2:
                    self.target_states[i] = 0  # Still full
                else:
                    self.target_states[i] = 1  # Half heart
        
        # Immediately update heart states for instant visual feedback
        self.heart_states = self.target_states.copy()
        
    def update(self, life):
        if life != self.life:
            self.previous_life = self.life
            self.life = max(0, min(100, life))  # Clamp between 0 and 100
            self.calculate_heart_states()
    ...
    def update_animation(self, dt):
        # Keep animation for smooth transitions if needed later
        self.animation_timer += dt
        if self.animation_timer >= self.animation_speed:
            self.animation_timer = 0
            
            # Update heart states towards target states
            for i in range(5):
    ...
    def draw(self, screen):
        heart_spacing = 5  # Reduced spacing since hearts are now properly sized
