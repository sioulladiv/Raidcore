...
class Game:
            ...
            if not self.show_book:
                ...
                for enemy in self.enemies:
                    ...
                    if player_rect.colliderect(enemy_rect) and self.player_damage_cooldown <= 0:
                        health_bar.damage(enemy.damage)  
    ...
    def next_level(self):
        self.enemies.clear()
        # Clear particles when transitioning levels
...
class Enemy:
    def __init__(self, x, y, Ennemytype='chort'):
        ...
        # Add velocity system for smooth movement
        self.velocity_x = 0.0
        self.velocity_y = 0.0
        ...
        self.damage_cooldown_duration = 60  
    ...
    def update(self, keys, tiles, player, enemies, collision_grid, bullets=None, health_bar=None, particles=None): 
            ...
            if distance_to_player < self.direct_movement_distance:
                ...
                if hasattr(self, 'path') and self.path and len(self.path) > 0:
                    ...
                    if self.path_arr_index < len(self.path):
                        ...
                        if (direction_x) ** 2 + (direction_y) ** 2 < 25:  # Reduced threshold for smoother movement
                            self.path_arr_index += 1
                            # If we've reached the end, try to move directly toward player
                            if self.path_arr_index >= len(self.path):
                                # Switch to direct movement when path ends
                                direction_x = (player.x + player.width // 2) - (self.x + self.width // 2)
                                direction_y = (player.y + player.height // 2) - (self.y + self.height // 2)
                            else:
                        ...
                        # Normalize and set desired velocity
                        distance_to_target = (direction_x ** 2 + direction_y ** 2) ** 0.5
                        if distance_to_target > 0:
                            desired_velocity_x = (direction_x / distance_to_target) * self.max_velocity
                            desired_velocity_y = (direction_y / distance_to_target) * self.max_velocity
                else:
                    # No path available - move directly toward player as fallback
                    direction_x = (player.x + player.width // 2) - (self.x + self.width // 2)
                    direction_y = (player.y + player.height // 2) - (self.y + self.height // 2)
                    distance_to_target = (direction_x ** 2 + direction_y ** 2) ** 0.5
                    if distance_to_target > 0:
        ...
        # Increase acceleration for more responsive movement
        ...
        # Apply friction when no desired movement
        if abs(desired_velocity_x) < 0.1:
            self.velocity_x *= 0.85  # Increased friction for more responsive stopping
    ...
    def update_pathfinding(self, player, collision_grid):
        ...
        try:
            ...
            # If pathfinding fails, clear the path to use direct movement
            self.path = []
            self.path_arr_index = 0
    ...
    def move_and_collide(self, dx, dy, tiles, enemies, bullets=None, player=None, health_bar=None, particles=None):
        ...
        if bullets:
            ...
            for bullet in list(bullets):
                ...
                if enemy_rect.colliderect(bullet_rect) and bullet.alive:
                    ...
                    bullet.alive = False
                    self.hit_timer = 20
                    print(f"Enemy hit! Lives remaining: {self.lives}")
                    ...
                    if self.lives <= 0:
                        if self in enemies:
                            ...
                            if particles is not None:
                                print(f"Creating {8} particles")
                                for i in range(8): 
                                    particles.append(Particle(
                                        self.x + self.width // 2, 
                                        self.y + self.height // 2, 
                                        random.uniform(-5, 5),  # Increased velocity for visibility
                                        random.uniform(-5, 5), 
                                        (255, 100, 0),  # Orange/red color for better visibility
                                        random.randint(1, 3),  # Larger particles
                                        random.randint(30, 60)  # Much shorter life time
                                    ))
        ...
        # Check enemy-to-enemy collisions for horizontal movement
        ...
        # Check enemy-to-enemy collisions for vertical movement
        for other_enemy in enemies:
            if other_enemy != self:
                other_rect = other_enemy.get_rect(other_enemy.x, other_enemy.y)
                if enemy_rect.colliderect(other_rect):
...
class TiledMap:
    ...
    def make_map(self):
        ...
        map_surface.fill((0, 0, 0))  # Fill with black background
...
class Particle:
    def __init__(self, x, y, velocity_x, velocity_y, color, size, life_time):
        ...
        self.gravity = 0.1  # Slightly more gravity for faster fall
        self.friction = 0.95  # Less friction so they move more before disappearing
    ...
    def update(self, dt):
        ...
        # Decrease life time faster
        self.life_time -= dt / 8.0  # Double the rate they disappear
        
        life_ratio = max(0, self.life_time / self.max_life_time)
        self.size = max(1, int(self.max_size * life_ratio))  # Allow smaller minimum size
    ...
    def draw(self, surface, camera):
        ...
        life_ratio = max(0, self.life_time / self.max_life_time)
        alpha = max(30, int(255 * life_ratio))  # Lower minimum alpha so they fade out more
        ...
        try:
            pygame.draw.circle(surface, self.color, (screen_x, screen_y), actual_size)
            if actual_size > 2:  # Lower threshold for white center
                pygame.draw.circle(surface, (255, 255, 255), (screen_x, screen_y), max(1, actual_size // 3))
        except ValueError:
            pass
